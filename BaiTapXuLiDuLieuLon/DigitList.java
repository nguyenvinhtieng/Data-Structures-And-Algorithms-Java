public class DigitList {
    ...
    public static DigitList subDigitLists(int c, DigitList L1, DigitList L2) {
        if ((L1 == null) && (L2 == null))
			...
			int t = c + L2.getDigit();
            return trimDigitList(new DigitList(t % 10, subDigitLists(t / 10, null , L2.getNextDigit())));
		}
        else if (L2 == null){
			if(L1.getDigit() >= c ) {
				int t = L1.getDigit() - c;
			...
			if(L1.getDigit() >= (L2.getDigit() + c) ) {
				...
				int t = L1.getDigit() + 10 - L2.getDigit() - c ;
	...
	public static DigitList digitize(String str) {
        if (str.indexOf("^") != -1) {
			...
			for (int i = 0; i < x.length(); i++) {
                K = new DigitList(x.charAt(i) - '0', K);
            }
			K = trimDigitList(K);
			return BigInteger.factorial(new BigInteger(K)).getDigits();
        } else {
            DigitList L = null;
